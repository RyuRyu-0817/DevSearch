    ...
    <header class="z-50 flex items-center justify-between py-8 md:py-8 mx-auto max-w-screen-2xl px-4 lg:px-28 bg-white sticky top-0" >
        ...
        <div v-if="store.state.login_user">
            <div class="-ml-8 hidden flex-col gap-2.5 sm:flex-row sm:justify-center lg:flex lg:justify-start">
                ...
                <p class="inline-block rounded-lg text-center text-sm font-semibold text-gray-500 outline-none ring-indigo-300 transition duration-100 hover:text-indigo-500 focus-visible:ring active:text-indigo-600 md:text-base">{{ store.state.login_user.username }}さん</p>
            </div>
        </div>
        <div v-else>
            <div class="-ml-8 hidden flex-col gap-2.5 sm:flex-row sm:justify-center lg:flex lg:justify-start">
